...
// --- Konfigurasi Tripay (IDEALNYA DI FILE config.php TERPISAH) ---
$apiKey = "20Tc8Y8CUXT8BWNCnv8RYDnfOgwo";
$privateKey = "AG53I-w5uBC-Ijmak-RAeXV-Kuvpn";
$merchantCode = "T41755";
// -----------------------------------------------------------------

// --- Database Connections ---
$connPemasangan = null; // Initialize to null
$connUmumData = null; // Initialize to null
...
try {
    ...
    // Set a user-friendly alert, but allow the form to load (perhaps without dropdown data)
    $alertType = 'danger';
    $alertMessage = "Terjadi kesalahan koneksi ke database. Beberapa fitur mungkin tidak berfungsi.";
    // IMPORTANT: If connection fails, $connPemasangan and $connUmumData will remain null.
    // Subsequent code MUST check for null before using these connections.
}
...
if ($connPemasangan) { // <-- ADD THIS CHECK
    ...
    if ($resultPop) {
        ...
        $alertType = 'danger'; // Add alert for the user
        $alertMessage .= "<br>Gagal mengambil data POP. Mohon coba lagi nanti.";
    }
} else {
    $alertType = 'danger'; // Ensure an alert is set if DB conn was already null
    $alertMessage .= "<br>Tidak dapat mengambil data POP karena koneksi database gagal.";
}
...
if ($connUmumData) { // <-- ADD THIS CHECK
    ...
    if ($resultPaket) {
        ...
        $alertType = 'danger'; // Add alert for the user
        $alertMessage .= "<br>Gagal mengambil data paket. Mohon coba lagi nanti.";
    }
} else {
    $alertType = 'danger'; // Ensure an alert is set if DB conn was already null
    $alertMessage .= "<br>Tidak dapat mengambil data paket karena koneksi database gagal.";
}

// Mapping grup WhatsApp per POP (tetap sama)
function getGroupIdForPop($popName) {
    $groups = [
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($alertMessage)) {
        // ... (pengambilan dan sanitasi variabel input tetap sama) ...
...
";
        ...
        if (function_exists('isPaketProrata') && isPaketProrata($idPaket)) {
            ...
            // Calculate prorata period more accurately (tetap sama)
            if ($today < $tgl5) {
            ...
            // SIAPKAN LINK KE pilih_pembayaran.php
            $linkKePilihPembayaran = "pilih_pembayaran.php?" . http_build_query([
                'nama'  => $nama,
                'paket' => $namaPaket, // Menggunakan nama paket, bukan ID, untuk ditampilkan di pilih_pembayaran.php
                'telp'  => $telp,
                'email' => $email,
                'harga' => $hargaProrata,
                // Anda bisa menambahkan 'periode_awal' dan 'periode_akhir' jika ingin menampilkannya di pilih_pembayaran.php
                // 'periode_awal' => $periodeAwal,
                // 'periode_akhir' => $periodeAkhir,
            ]);

            if (function_exists('waNotifProrata')) {
                // waNotifProrata sekarang mengirim link ke pilih_pembayaran.php
                waNotifProrata($telp, $nama, $namaPaket, $hargaProrata, $periodeAwal, $periodeAkhir, $linkKePilihPembayaran);
            }

            $alertType = 'success';
            $alertMessage = "Permintaan pemasangan berhasil diajukan! Link pembayaran sudah dikirim ke WhatsApp pelanggan.";
            $alertMessage .= "<br><b>Link Pembayaran (Admin):</b> <a href='$linkKePilihPembayaran' target='_blank'>$linkKePilihPembayaran</a><br>";
            $alertMessage .= "<small>Jika pelanggan tidak menerima WA, bisa copy-paste link ini ke pelanggan.</small>";

        } else {
            // --- WA Notifikasi pelanggan normal (tanpa Tripay/prorata) (tetap sama)
            $bodyCust = "
👋 Halo {$nama}!
...
// Close database connections at the end (tetap sama)
if ($connPemasangan) {
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function clearForm() {
                form.reset();
                // Reset select options to their default/first disabled selected state
                document.getElementById('paket').selectedIndex = 0;
                document.getElementById('pop').selectedIndex = 0;
            }

            // Set alert to disappear automatically after 5 seconds if it's a success message
            if (alertBox && alertBox.classList.contains('alert-success')) {
                setTimeout(function() {
                    ...
                    clearForm(); // Clear form only on success
                }, 5000);
            }
        });
    </script>
